...
import { contractorSignupNotificationEmail } from '../_lib/emailTemplates.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const primaryTrade = clean(req.body?.primaryTrade, 100);
  ...
  try {
    const { error } = await supabase.from('early_access_contractors').insert({
      name,
      email,
      company_name: companyName || null,
      primary_trade: primaryTrade || null,
      location: location || null,
      source: 'early-access-contractors',
    });
  ...
  try {
    const tpl = contractorSignupNotificationEmail({
      name,
      email,
      companyName,
      primaryTrade,
    ...
    console.error('[EarlyAccess] Notification email failed:', err);
  }
  ...
  console.log(`[EarlyAccess] Contractor signup — captured:${captured} notified:${notified} email:${email}`);
