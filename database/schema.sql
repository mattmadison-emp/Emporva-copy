...
CREATE TABLE public.emporva_documents (
  id text NOT NULL,
  url text NOT NULL,
  title text NOT NULL,
  ...
  CONSTRAINT emporva_documents_pkey PRIMARY KEY (id)
);
CREATE TABLE public.emporva_embeddings (
  id text NOT NULL,
  reasoning_unit_id text NOT NULL,
  content_text text NOT NULL,
  embedding USER-DEFINED,
  metadata jsonb DEFAULT '{}'::jsonb,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT emporva_embeddings_pkey PRIMARY KEY (id),
  CONSTRAINT emporva_embeddings_reasoning_unit_id_fkey FOREIGN KEY (reasoning_unit_id) REFERENCES public.emporva_reasoning_units(id)
);
CREATE TABLE public.emporva_pipeline_runs (
  id integer NOT NULL DEFAULT nextval('emporva_pipeline_runs_id_seq'::regclass),
  ...
  metadata jsonb DEFAULT '{}'::jsonb,
  CONSTRAINT emporva_pipeline_runs_pkey PRIMARY KEY (id)
);
CREATE TABLE public.emporva_reasoning_units (
  id text NOT NULL,
  source_document_ids jsonb DEFAULT '[]'::jsonb,
  source_category text NOT NULL,
  source_names jsonb DEFAULT '[]'::jsonb,
  symptom text NOT NULL,
  symptom_keywords jsonb DEFAULT '[]'::jsonb,
  clarifying_questions jsonb DEFAULT '[]'::jsonb,
  safety_actions jsonb DEFAULT '[]'::jsonb,
  likely_causes jsonb DEFAULT '[]'::jsonb,
  trades_required jsonb DEFAULT '[]'::jsonb,
  diy_feasibility text DEFAULT 'low'::text,
  urgency text DEFAULT 'routine'::text,
  workflow_sequence jsonb DEFAULT '[]'::jsonb,
  material_categories jsonb DEFAULT '[]'::jsonb,
  escalation_triggers jsonb DEFAULT '[]'::jsonb,
  related_symptoms jsonb DEFAULT '[]'::jsonb,
  home_system text DEFAULT ''::text,
  component text DEFAULT ''::text,
  estimated_lifespan_years integer,
  confidence_score double precision DEFAULT 0.0,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT emporva_reasoning_units_pkey PRIMARY KEY (id)
);
CREATE TABLE public.emporva_service_providers (
  id text NOT NULL,
  user_id uuid NOT NULL,
  ...
  CONSTRAINT emporva_service_providers_pkey PRIMARY KEY (id),
  CONSTRAINT emporva_service_providers_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);